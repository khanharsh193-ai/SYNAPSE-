...
async function sendChatMessage() {
    ...
    try {
        ...
        hideTyping();
        addMessage('ai', response, true);  // true = animate with typewriter
        isTyping = false;
        updateStreak();
    } catch (error) {
        hideTyping();
...
function addMessage(role, content, animate = false) {
    ...
    msgDiv.innerHTML = `
        ...
        <div class="bubble"><p class="message-text"></p></div>
    `;
    ...
    if (animate && role === 'ai') {
        ...
        typeWriter(textEl, content, 0);
    } else {
        // Instant display for user messages or non-animated AI
        let formattedContent = content
            .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
            .replace(/\*(.*?)\*/g, '<em>$1</em>')
            .replace(/\n\n/g, '</p><p>')
            .replace(/\n/g, '<br>');
        msgDiv.querySelector('.bubble').innerHTML = `<p>${formattedContent}</p>`;
    }
...
function typeWriter(element, text, index) {
    if (index < text.length) {
        // Get the raw text up to current index and format it
        const rawText = text.substring(0, index + 1);
...
CRITICAL RULE - NEVER ASK WHAT TO STUDY:
When a student opens a specific chapter, you MUST start teaching that chapter IMMEDIATELY. Do NOT ask "What would you like to learn?" or "What do you want to know?" or any variation of that question. The student has already chosen the chapter by clicking on it. Your job is to START TEACHING right away.

Teaching Structure (ALWAYS follow this):
1. THE HOOK: Start with something the student has experienced in daily life in India
2. THE CONCEPT: Introduce the idea simply, using plain language first
3. THE VISUALIZATION: Make them picture it - use motion, scale, detail
4. THE REALITY: Connect to their world - home, school, market, playground
5. THE WHY: Explain why this concept exists and what problem it solves
6. THE CHECK: Ask ONE thinking question (not a memory test)
7. THE THREAD: End with a hint about what's coming next

Other Rules:
- Explain concepts simply, as if talking to an 11-12 year old student
- Use Indian examples and context where possible
- Be encouraging and patient
- For math, show step-by-step working
- For science, connect to real-world observations
- For social science, use stories and relatable examples
- If unsure, say "Let me check that for you" rather than guessing
- Keep responses concise but complete (3-5 paragraphs max)
- Use bullet points for lists
- NEVER ask "What do you want to learn?" - the student already chose`;
...
function renderRecentChats() {
    ...
    container.innerHTML = recent.map(title => {
        ...
        return `
        <a href="#" onclick="continueRecentChat('${safeTitle}')" 
...
function continueRecentChat(title) {
    // Extract subject and chapter from the stored title format "Subject: Chapter Title"
    const parts = title.split(': ');
    if (parts.length >= 2) {
        const subjectName = parts[0];
        const chapterTitle = parts.slice(1).join(': ');

        // Find the subject key
        let subjectKey = null;
        if (subjectName === 'Mathematics') subjectKey = 'mathematics';
        else if (subjectName === 'Science') subjectKey = 'science';
        else if (subjectName === 'Social Science') subjectKey = 'social';

        if (subjectKey) {
            // Find chapter number
            const chapter = NCERT_DATA[subjectKey].chapters.find(ch => ch.title === chapterTitle);
            if (chapter) {
                const chNum = typeof chapter.num === 'number' ? chapter.num : chapter.num;
                window.location.href = `index.html?subject=${subjectKey}&chapter=${chNum}&title=${encodeURIComponent(chapterTitle)}`;
                return;
            }
        }
    }

    // Fallback: just open general chat
