...
function showSubjectsView() {
    ...
    if (subjectsView && chaptersView) {
        chaptersView.classList.add('section-hidden');
        subjectsView.classList.remove('section-hidden');
        // Scroll to top
        subjectsView.scrollTop = 0;
    }
}
...
function showChaptersView() {
    ...
    if (subjectsView && chaptersView) {
        subjectsView.classList.add('section-hidden');
        chaptersView.classList.remove('section-hidden');
        // Scroll to top
        chaptersView.scrollTop = 0;
    }
}
...
function showSubjectChapters(subjectKey) {
    ...
    if (chaptersTitle) {
        chaptersTitle.innerHTML = `<span style="color: ${subject.colorHex}">${subject.name}</span> — Class 6`;
    }
    ...
    if (progressText) progressText.textContent = `${completed}/${total} chapters`;
    ...
    if (grid) {
        grid.innerHTML = subject.chapters.map(ch => {
            ...
            const btnClass = isCompleted 
                ...
                : 'bg-accent-green hover:bg-accent-hover text-dark-900 font-bold';
            const btnIcon = isCompleted ? 'lucide:rotate-ccw' : 'lucide:play';
            const btnText = isCompleted ? 'Review Chapter' : isInProgress ? 'Continue Learning' : 'Start Learning';
            ...
            <div class="chapter-card bg-dark-800 border border-dark-600 rounded-xl p-5 ${statusClass}" onclick="startChapter('${subjectKey}', '${chNum}', '${safeTitle}')">
                ...
                <h4 class="font-semibold mb-2 text-white">${ch.title}</h4>
                ...
                <button class="w-full py-2.5 rounded-lg ${btnClass} transition-all text-sm flex items-center justify-center gap-2">
                    <iconify-icon icon="${btnIcon}" class="text-sm"></iconify-icon>
                    ${btnText}
                </button>
            </div>
            `;
